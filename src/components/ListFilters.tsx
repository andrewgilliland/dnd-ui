      ...
      <div className="grid gap-3 md:grid-cols-3">
        <label className="flex flex-col gap-1 text-sm text-slate-700">
          ...
          <input
            ...
            className="rounded-md border border-slate-300 px-3 py-2 text-slate-900 placeholder:text-slate-400"
          />
        </label>
        ...
        {selectFilters.map((filter) => (
          ...
          >
            ...
            <select
              value={filter.value}
              onChange={(event) => onSelectChange(filter.key, event.target.value)}
